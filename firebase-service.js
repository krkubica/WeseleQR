...
export async function connectGuest(state) {
  ...
  await Promise.all([setDoc(participantRef,{...payload(state),createdAt:snapshot.exists()?undefined:serverTimestamp()},{merge:true}),setDoc(publicProfileRef,profilePayload(state),{merge:true})]);
...
export async function uploadPhoto(file,state,task,caption='') { if(!auth?.currentUser||!storage||!db)throw new Error('Brak połączenia z galerią.');const ext=(file.name.split('.').pop()||'jpg').replace(/[^a-z0-9]/gi,'').toLowerCase()||'jpg',path=`photos/${auth.currentUser.uid}/${Date.now()}-${Math.random().toString(36).slice(2,8)}.${ext}`;const storageRef=ref(storage,path);await uploadBytes(storageRef,file,{contentType:file.type});const imageUrl=await getDownloadURL(storageRef);await addDoc(collection(db,'photos'),{imageUrl,storagePath:path,nickname:state.nickname||'Gość',caption,taskId:task?.id||null,taskText:task?.text||'',ownerId:auth.currentUser.uid,createdAt:serverTimestamp()}); }
